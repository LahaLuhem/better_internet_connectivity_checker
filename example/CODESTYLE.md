...
Prose here follows [Prose & voice](../CODESTYLE.md#prose), same as anywhere else in the repo. Read
<https://noslopgrenade.com/> before writing any.
...
The example uses [pmvvm](https://pub.dev/packages/pmvvm), where
...
- **No `notifyListeners()`.** Every state-mutating method writes
  ...
  `viewBuilder` becomes a static frame. pmvvm earns its keep as DI plus lifecycle, not as
  a rebuild trigger.
- **Naming: `_xNotifier` for the private field, `xListenable` for the public getter.**
  ...
  bare-noun field from colliding with the getter name. The view binds the getter, and it
  cannot mutate.
...
- **Group co-updated fields into one notifier with a record type.** Fields that are
  always written together, `(status, targetUrl)` or
...
- ★ Default to `should` for user preferences and UI toggle state, being declarative,
...
a `shouldXxx` field, because the event and the state describe different things.
...
Avoid VM-leaking names like `setX`, `runX`, `commitX`, `forceX`, which describe what
...
4. **`init()`**: lifecycle entry, sets up streams and triggers.
5. **Getters**: the `xListenable` getters and any other pure reads.
6. **Getter-like methods**: pure or near-pure reads expressed as methods (rare).
...
- **Widget-state holding domain input belongs on the VM.** `TextEditingController`,
  ...
  disposal, and the view binds directly (`controller: viewModel.urlController`). They ARE
...
- **Widget-state describing pure UI presentation belongs on the view.** "This button
  is mid-async, show a spinner" is purely visual, with no VM logic and no other widget
  consume it. Use `tap_debouncer` (via `AsyncIconActionButton`) so the view tracks its
  own in-flight gate. Do NOT add an `isRunning` field on the VM for this, which was
...
- **`PlatformCard` content `Padding`: `.all(16)`** by default, which matches Material 3's
...
iOS from a single widget tree. 3 things the library's surface does NOT enforce,
...
- **Plug missing widgets under [`platform/`](lib/features/core/widgets/platform/), one
  ...
  leaving a Material-only widget in the tree. These are temporary, moving into the
  base library later.
...
- **Icons: `PlatformIcon(PlatformIcons.x)` where
  ...
  glyph. The 2 resolve platform differently (`PlatformIcon` via `dart:io Platform`,
...
- **Colours: define them as `ConstTheme.<name>(context)` methods, never inline
  ...
  blessed selector for cheap values like colours, enums and `IconData`) in
...
- **Import `platform_adaptive_widgets` and `platform_icons` wholesale, no `show`.**
  ...
  be reachable, and a `show` list buries that and goes stale every time a screen reaches for
  one more `Platform*` widget. Reserve `show` for 2 cases only: (1) `material_ui` /
  ...
  no collision today between the 2 packages (`PlatformIcon` / `PlatformIcons` /
  `PlatformTarget`), `material_ui`, or `cupertino`.

Verify iOS-only layout in `flutter test` with
`debugDefaultTargetPlatformOverride = TargetPlatform.iOS`, reset in a `finally`,
