...
7. **No manual `CHANGELOG.md`, `version:`, or `example/pubspec.lock` edits; no hand-run
   ...
   or overwritten. Curate the `## [Unreleased]` section of `CHANGELOG.md` by hand between
   releases — the script consumes it. Keep the bracket-and-link-reference format
   consistent with the already-released entries (`## [Unreleased]` heading,
   `[Unreleased]: …/compare/<last-tag>...vHEAD` reference at the bottom of the file,
   `### Added` / `### Changed` / `### Fixed` subsections) — that is the shape cider's
   `link_template` expects; inconsistent headings get rewritten on release. The `cider:`
   block in `pubspec.yaml` is static configuration (link templates, URLs) and may be
   hand-edited. `example/pubspec.lock` is regenerated by the release script's
   `flutter pub get` step in `example/` — never bump or hand-resolve it; the script does
   that as part of each release.
   *Bot exception:* the post-merge automation in
   [`.github/workflows/changelog.yml`](../.github/workflows/changelog.yml) runs
   `cider log` to append the merged-PR title under `## [Unreleased]`. That is the only
   sanctioned non-script invocation; humans and other agents still must not.
...
- **Exactly one `sem-*` label per PR.** Selects the changelog category for the
  ...
  |-----------------|--------------|------------------------------------------------|
  | `sem-add`       | `added`      | New public symbol / feature                    |
